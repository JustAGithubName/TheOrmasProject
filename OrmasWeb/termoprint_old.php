...
<?php
	...
	{	
		...
		else
		{
			...
			{
				$balance_client = "SELECT * FROM \"OrmasSchema\".balances_view WHERE user_id=".$order_row['user_id'];
				$result_balance_client = pg_query($balance_client);
				if(!$result_balance_client)
				{
					$string_out = $string_out."Ошибка! Обратитесь к администратору. Нет клиента";
				}
				else
				{
					$balance_row = pg_fetch_array($result_balance_client);
					$client_subaccount = "SELECT * FROM \"OrmasSchema\".subaccounts_view WHERE subaccount_id=".$balance_row['subaccount_id'];
					$result_client_subaccount = pg_query($client_subaccount);
					if(!$result_client_subaccount)
					{
						$string_out = $string_out."Ошибка! Обратитесь к администратору. Нет счета клиента";
					}
					$subaccount_row = pg_fetch_array($result_client_subaccount);
				}
				...
				<div id = 'row' width='50%' style='font-family:\" Garamond\", Garamond, monospace; font-size:180%'>
					...
					Остаток до:<b> ".(float)$subaccount_row['current_balance']." ".$order_row['currency_name']."</b>
				</div>
				<br/><br/>
				<div id='content'>
				...
				<div id = 'row' width='50%' style='font-family:\" Garamond\", Garamond, monospace; font-size:200%'>
					Остаток после:<b> ".((float)$total_sum + (float)$subaccount_row['current_balance'])." ".$order_row['currency_name']."</b>
				</div>
				</br></br>
	...
	if(isset($_POST['return-id']))
	{
		...
		else
		{
			...
			{
				$balance_client = "SELECT * FROM \"OrmasSchema\".balances_view WHERE user_id=".$return_row['user_id'];
				$result_balance_client = pg_query($balance_client);
				if(!$result_balance_client)
				{
					$string_out = $string_out."Ошибка! Обратитесь к администратору. Нет клиента";
				...
				<div id = 'row' width='50%' style='font-family:\" Garamond\", Garamond, monospace; font-size:180%'>
					...
					Остаток до:<b> ".(float)$subaccount_row['current_balance']." ".$return_row['currency_name']."</b>
				</div>
				</br></br>
				<div id='content'>
					<table width = '100%' style='font-family:\" Garamond\", Garamond, monospace; font-size:160%' class='reset-style simple'  border = 0px  cellpadding=5 style='border-spacing:0px;'>
				...
				<div id = 'row' width='50%' style='font-family:\" Garamond\", Garamond, monospace; font-size:200%'>
					Остаток после:<b> ".((float)$subaccount_row['current_balance']-(float)$total_sum)." ".$order_row['currency_name']."</b>
				</div>
				</br></br>
