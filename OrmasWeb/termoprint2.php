		...
		else
		{
			...
			else
			{
				...
				$string_out = $string_out."<div id='report-header' >
					<div id='report-name' style='font-family:\" Garamond\", Garamond, monospace; font-size:200%'>
						<b>Накладная</b><b> №</b><b>".$_POST['order-id']."</b>
					</div>
					<div id='report-period' style='font-family:\" Garamond\", Garamond, monospace; font-size:150%'><b>
						на ".$order_row['order_date']
					."</b></div>
				</div>
				</br>
				<div id = 'row' width='50%' style='font-family:\" Garamond\", Garamond, monospace; font-size:160%'>
					Получатель:</br> ".$order_row['client_name']." ".$order_row['client_surname']."
				</div>
				</br>
				</br>
				<div id = 'row' width='50%' style='font-family:\" Garamond\", Garamond, monospace; font-size:160%'>
				...
				<div id='content'>
					<table style='font-family:\" Garamond\", Garamond, monospace; font-size:170%' class='reset-style simple'  border = 0px  cellpadding=5 style='border-spacing:0px;'>
				...
				while ($row = pg_fetch_array($result_order_list)) {
					...
					$sum=$row['sum']+0;
					$price=$row['price']+0;
					$string_out = $string_out."<tr class='simple-data-print'><td class='simple-data-print'><b>".$counter.") ".$row['product_name']."</b></td>
	...
	if(isset($_POST['return-id']))
	{
		...
		else
		{
			...
			else
			{
				...
				<div id = 'row' width='50%' style='font-family:\" Garamond\", Garamond, monospace; font-size:160%'>
					Отправитель: ".$return_row['client_name']." ".$return_row['client_surname']."
				</div>
				</br></br>
				<div id='content'>
					<table style='font-family:\" Garamond\", Garamond, monospace; font-size:170%' class='reset-style simple'  border = 0px  cellpadding=5 style='border-spacing:0px;'>
	...
	if(isset($_POST['order-raw-id']))
	{
		...
		else
		{
			...
			else
			{
				...
				<div id='content'>
					<table width = '100%' style='font-family:\" Garamond\", Garamond, monospace; font-size:140%' class='reset-style simple'  border = 0px  cellpadding=5 style='border-spacing:0px;'>
						<th class='simple-data-print'><b>Наименование</b>&nbsp&nbsp&nbsp&nbsp<b>Количество * </b><b>Цена = </b><b>Сумма</b></th>
						";
				while ($row = pg_fetch_array($result_order_raw_list)) {
					...
					$sum=$row['sum']+0;
					$price=$row['price']+0;
					$string_out = $string_out."<tr class='simple-data-print'><td class='simple-data-print'>".$row['product_name']."</td></tr>
						<tr class='simple-data-print' align= 'right'><td align= 'right' class='simple-data-print'>".$count." ".$row['measure_short_name']." 
						 * ".$sum/$count." = ".$sum."</td></tr>";
					$count=0;
					$sum=0;
					$price=0;
