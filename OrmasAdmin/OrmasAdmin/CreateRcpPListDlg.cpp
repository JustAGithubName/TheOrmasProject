...
CreateRcpPListDlg::CreateRcpPListDlg(BusinessLayer::OrmasBL *ormasBL, bool updateFlag, QWidget *parent) :QDialog(parent)
{
	...
	QObject::connect(countEdit, &QLineEdit::textChanged, this, &CreateRcpPListDlg::TextEditChanged);
	QObject::connect(sumEdit, &QLineEdit::textChanged, this, &CreateRcpPListDlg::TextEditChanged);
	InitComboBox();
}
...
void CreateRcpPListDlg::SetRcpPListParams(int rReceiptProductID, int rProductID, double rCount, double rSum, int rStatusID, int rCurrencyID, int id)
{
...
void CreateRcpPListDlg::FillEditElements(int rReceiptProductID, int rProductID, double rCount, double rSum, int rStatusID, int rCurrencyID)
{
...
void CreateRcpPListDlg::EditProductInList()
{
	...
	if (0 != productEdit->text().toInt() || 0 != countEdit->text().toDouble())
	{
		if (productEdit->text().toInt() != receiptProductList->GetProductID() || countEdit->text().toDouble() != receiptProductList->GetCount()
			|| receiptProductEdit->text().toInt() != receiptProductList->GetReceiptProductID() || sumEdit->text().toDouble() != receiptProductList->GetSum()
			|| statusEdit->text().toInt() != receiptProductList->GetStatusID() || currencyCmb->currentData().toInt() != receiptProductList->GetCurrencyID())
		{
			BusinessLayer::Product *product = new BusinessLayer::Product();
			if (!product->GetProductByID(dialogBL->globalVar, dialogBL->GetOrmasDal(), productEdit->text().toInt(), errorMessage))
			{
...
void CreateRcpPListDlg::InitComboBox()
{
...
void CreateRcpPListDlg::TextEditChanged()
{
