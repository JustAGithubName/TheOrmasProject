...
CreateWOffRListDlg::CreateWOffRListDlg(BusinessLayer::OrmasBL *ormasBL, bool updateFlag, QWidget *parent) :QDialog(parent)
{
	...
	QObject::connect(countEdit, &QLineEdit::textChanged, this, &CreateWOffRListDlg::TextEditChanged);
	QObject::connect(sumEdit, &QLineEdit::textChanged, this, &CreateWOffRListDlg::TextEditChanged);
...
void CreateWOffRListDlg::SetWOffRListParams(int wWriteOffRawID, int wProductID, double wCount, double wSum, int wStatusID, int wCurrencyID, int id)
{
...
void CreateWOffRListDlg::FillEditElements(int wWriteOffRawID, int wProductID, double wCount, double wSum, int wStatusID, int wCurrencyID)
{
	writeOffRawEdit->setText(QString::number(wWriteOffRawID));
	productEdit->setText(QString::number(wProductID));
	countEdit->setText(QString::number(wCount, 'f', 3));
			...
			if (parentDataForm != nullptr)
			{
				if (!parentDataForm->IsClosed())
				{
					...
					productListItem << new QStandardItem(product->GetName().c_str())
						...
						<< new QStandardItem(QString::number(writeOffRawList->GetCount(), 'f', 3))
						<< new QStandardItem(QString::number(writeOffRawList->GetSum(),'f',3))
...
void CreateWOffRListDlg::EditProductInList()
{
	...
	if (0 != productEdit->text().toInt() || 0 != countEdit->text().toDouble())
	{
		if (productEdit->text().toInt() != writeOffRawList->GetProductID() || countEdit->text().toDouble() != writeOffRawList->GetCount()
			|| writeOffRawEdit->text().toInt() != writeOffRawList->GetWriteOffRawID() || sumEdit->text().toDouble() != writeOffRawList->GetSum()
			...
			if (dialogBL->UpdateWriteOffRawList(writeOffRawList, errorMessage))
			{
				if (parentDataForm != nullptr)
				{
					if (!parentDataForm->IsClosed())
					{
						...
						itemModel->item(mIndex.row(), 7)->setText(QString::number(writeOffRawList->GetCount(), 'f', 3));
						itemModel->item(mIndex.row(), 8)->setText(QString::number(writeOffRawList->GetSum(),'f',3));
						...
						itemModel->item(mIndex.row(), 11)->setText(QString::number(writeOffRawList->GetProductID()));
						itemModel->item(mIndex.row(), 12)->setText(QString::number(writeOffRawList->GetStatusID()));
...
void CreateWOffRListDlg::InitComboBox()
{
...
void CreateWOffRListDlg::TextEditChanged()
{
