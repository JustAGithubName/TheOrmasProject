			...
			if (parentDataForm != nullptr)
			{
				if (!parentDataForm->IsClosed())
				{
					...
					ProductListItem << new QStandardItem(product->GetName().c_str())
						...
						<< new QStandardItem(QString::number(returnList->GetCount(), 'f', 3))
						<< new QStandardItem(QString::number(returnList->GetSum(),'f',3))
			...
			if (dialogBL->UpdateReturnList(returnList, errorMessage))
			{
				if (parentDataForm != nullptr)
				{
					if (!parentDataForm->IsClosed())
					{
						...
						itemModel->item(mIndex.row(), 7)->setText(QString::number(returnList->GetCount(), 'f', 3));
						itemModel->item(mIndex.row(), 8)->setText(QString::number(returnList->GetSum(),'f',3));
